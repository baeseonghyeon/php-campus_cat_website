...
$res = mysql_query($sql);
// 좋아요 회수 얻기 
$sql = "select count(*) as cnt from c_like where cidx = $idx";
$res = mysql_query($sql);
$clike = mysql_fetch_array($res);
?>
...
<body>
	...
	<h2><?=$row[title]?></h2>
	<p><?=$row[user_id]?> | <?=$row[reg_time]?> | <?=$row[hit]?> | <img id="c_like"><span id="clcnt"><?=$clike[cnt]?></span></p>
		<? if( $row[img] ) { ?>
